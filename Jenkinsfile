  ...
  stages {
    ...
    stage('Run') {
     steps {
      ...
      sh 'docker run -d --net selenoid --name selenoid -p 4445:4444 v $(pwd):/etc/selenoid/:ro -v /var/run/docker.sock:/var/run/docker.sock aerokube/selenoid:latest-release -conf /etc/selenoid/browsers.json -video-output-dir /opt/selenoid/video/ -timeout 3m0s -container-network selenoid'
